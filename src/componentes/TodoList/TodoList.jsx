...
const TodoList = () => {
    ...
    //1) Función para borrar TODOS: 
    const borrarTodo = (todoABorrar) => {
        const actualizarTodos = todos.filter(tarea => tarea !== todoABorrar);
        setTodos(actualizarTodos);
    }

    //2) Función para agregar un Todo nuevo: 
    const agregarTodo = (texto) => {
        setTodos([...todos, texto]);
        //Simplemente usamos el operador spread para agregar nuevas tareas en lugar del push. 
    }

    //3)Función para manejar los eventos del formulario: 
    ...
    return (
        <div>
            ...
            <form onSubmit={manejadorSubmit}>
                <input type="text" placeholder="Ingrese una tarea" onChange={(e)=> setInputValue(e.target.value)} value={inputValue} />
